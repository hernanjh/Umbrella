...
export default function CategoriesPage() {
  ...
  const { data: categories, isLoading, refetch } = useQuery({ queryKey: ['categories'], queryFn: paramsService.getCategories })

  const saveMutation = useMutation({
    mutationFn: (d: any) => modal.data?.id ? paramsService.updateCategory(modal.data.id, d) : paramsService.createCategory(d),
    onSuccess: () => { toast.success('Guardado'); qc.invalidateQueries({ queryKey: ['categories'] }); setModal({ open: false }) },
    onError: (e: any) => toast.error(e.response?.data?.message || 'Error')
  })
  const deleteMutation = useMutation({ mutationFn: paramsService.deleteClientType, onSuccess: () => { toast.success('Eliminado'); qc.invalidateQueries({ queryKey: ['categories'] }) } })

  const columns: Column<any>[] = [
    { key: 'code', header: 'Código', width: '100px' },
    { key: 'name', header: 'Nombre' },
    { key: 'parentCategoryName', header: 'Categoría Padre' },
    { key: 'isActive', header: 'Estado', render: r => <Badge variant={r.isActive ? 'green' : 'red'}>{r.isActive ? 'Activo' : 'Inactivo'}</Badge> },
  ]

  return (
    <div>
      <PageHeader title="Categorías" actions={<button className="btn-primary" onClick={() => { setForm({}); setModal({ open: true }) }}><Plus className="w-4 h-4" /> Nueva</button>} />
      <div className="card p-5">
        <DataGrid columns={columns} data={categories ?? []} loading={isLoading} onRefresh={refetch} exportFileName="categorias"
          actions={row => (<>
      ...
      <Modal open={modal.open} onClose={() => setModal({ open: false })} title={modal.data ? 'Editar Categoría' : 'Nueva Categoría'}
        ...
        <div className="space-y-3">
          ...
          <div className="form-group">
            ...
            <select className="input" value={form.parentCategoryId ?? ''} onChange={e => setForm((f: any) => ({ ...f, parentCategoryId: +e.target.value || null }))}>
              <option value="">Sin padre (raíz)</option>
              {(categories ?? []).filter((c: any) => c.id !== modal.data?.id).map((c: any) => <option key={c.id} value={c.id}>{c.name}</option>)}
            </select>
          </div>
