...
public interface IInvoicePdfService
{
    ...
    Task<byte[]> GeneratePurchaseInvoicePdfAsync(int invoiceId);
}
...
public class InvoicePdfService : IInvoicePdfService
{
    ...
    public async Task<byte[]> GeneratePurchaseInvoicePdfAsync(int invoiceId)
    {
        var inv = await _db.PurchaseInvoices
            .Include(i => i.Supplier).ThenInclude(s => s.VatCondition)
            .Include(i => i.PaymentCondition)
            ...
            doc.Page(page =>
            {
                ...
                page.Header().Column(col =>
                {
                    col.Item().Row(row =>
                    {
                        ...
                        row.RelativeItem().Column(c =>
                        {
                            c.Item().AlignRight().Text("ORDEN DE COMPRA / RECIBO").FontSize(14).Bold();
                            c.Item().AlignRight().Text($"Tipo: {inv.InvoiceType}");
                            c.Item().AlignRight().Text($"Nº {inv.FullNumber}");
                            c.Item().AlignRight().Text($"Comp. Proveedor: {inv.SupplierInvoiceNumber}");
                            c.Item().AlignRight().Text($"Fecha: {inv.InvoiceDate:dd/MM/yyyy}");
                            if (inv.DueDate != default) c.Item().AlignRight().Text($"Vencimiento: {inv.DueDate:dd/MM/yyyy}");
                            c.Item().AlignRight().Text($"Estado: {inv.Status}").FontColor(Colors.Grey.Darken2);
                        });
                ...
                page.Content().PaddingVertical(10).Column(col =>
                {
                    col.Item().PaddingBottom(8).Column(c =>
                    {
                        ...
                        c.Item().Text($"{inv.Supplier.BusinessName}");
                        c.Item().Text($"CUIT: {inv.Supplier.Cuit}");
                        if (inv.Supplier.VatCondition != null) c.Item().Text($"Cond. IVA: {inv.Supplier.VatCondition.Name}");
                        if (!string.IsNullOrWhiteSpace(inv.Supplier.Address)) c.Item().Text($"{inv.Supplier.Address}{(inv.Supplier.City != null ? $", {inv.Supplier.City}" : "")}");
                        if (inv.StockLocation != null) c.Item().Text($"Locación: {inv.StockLocation.Name}");
                        if (inv.PaymentCondition != null) c.Item().Text($"Cond. pago: {inv.PaymentCondition.Name}");
                    });
                    ...
                    col.Item().Table(t =>
                    {
                        t.ColumnsDefinition(c =>
                        {
                            c.ConstantColumn(60); c.RelativeColumn(3); c.ConstantColumn(60);
                            c.ConstantColumn(80); c.ConstantColumn(50); c.ConstantColumn(50); c.ConstantColumn(80);
                        });
                        t.Header(h =>
                        {
                            h.Cell().Background(Colors.Grey.Lighten2).Padding(4).Text("Código").Bold();
